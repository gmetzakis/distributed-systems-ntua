    ...
    if request.method == 'POST':
        ...
        if (rep==(k_rep-1)):    #done after this
            ...
            if replicas_method=="lin":
                ...
                '''
                print("I went in to forward the request "+result["Value"])
                @app.after_response
                def rep_forwarding():
                    print("After return just began for the request "+result["Key"])
                    requests.post(str(suc_ip+'/insert_replicas'), data = {'k_rep':(rep+1), 'Key':result["Key"], 'Value':result["Value"]})
                    print("After return just FINISHED for the request "+result["Key"])
                return render_template('messages.html', number=6, x = result["Key"], y=result["Value"], z=node_id)
                try:
                    return render_template('messages.html', number=6, x = result["Key"], y=result["Value"], z=node_id)
                finally:
...
if __name__ == '__main__':
    ...
    add_delay = int(sys.argv[6])
    if add_delay:
        time_sleep = float(sys.argv[7])
